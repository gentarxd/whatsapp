...
async function startSock(sessionId) {
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
        ...
        // لو انت رديت → Pause البوت لمدة ساعة
        if (isFromMe) {
        ...
        // لو فيه Pause شغال → تجاهل
        if (humanOverride[from] && Date.now() < humanOverride[from]) {
            console.log(`[AUTO-PAUSE] Ignored for ${from} (paused)`);
            return;
        }
        ...
        // --- إرسال Auto Reply (اختياري، أو يمكن تركه بدون نص)
        await sock.sendMessage(from, { text: "مرحباً! كيف أقدر أساعدك؟" });
...
// ---- reconnect existing sessions on boot
function reconnectSessions() {
    try {
        ...
        } else fs.mkdirSync(AUTH_DIR, { recursive: true });
    } catch (e) { console.error("reconnectSessions error", e?.message || e); }
}
...
async function processQueueItem(item) {
    ...
    if (!sock) { messageStatus[phone] = "no_session"; return; }
    const jid = phone.includes("@") ? phone : `${phone}@s.whatsapp.net`;
    if (!canAutoReply(jid)) { messageStatus[phone] = "paused"; return; }
    ...
    try {
        ...
        } else await sock.sendMessage(jid, { text: text || " " });
...
setInterval(async () => {
    ...
    try { await processQueueItem(item); } catch (e) { console.error(e); }
    saveQueue();
}, PROCESS_INTERVAL_MS);
...
// graceful shutdown
process.on("SIGINT", () => { console.log("SIGINT: saving state and exiting..."); saveOverrides(); saveQueue(); process.exit(0); });
process.on("SIGTERM", () => { console.log("SIGTERM: saving state and exiting..."); saveOverrides(); saveQueue(); process.exit(0); });  
